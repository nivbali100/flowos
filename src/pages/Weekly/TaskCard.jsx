...
import {
  ...
  Star, RotateCcw,
  AlertTriangle, Clock, Zap, Target, CalendarClock, ArrowLeftRight,
} from 'lucide-react'
...
function dueDateStatus(dueDate) {
  ...
  if (diff < 0)  return { label: `פג ${absDiff}${dayWord(absDiff)}`, color: 'text-red-600 bg-red-50',     overdue: true }
  if (diff === 0) return { label: 'היום',                              color: 'text-red-500 bg-red-50',     overdue: false }
  if (diff === 1) return { label: 'מחר',                               color: 'text-amber-600 bg-amber-50', overdue: false }
  if (diff <= 3)  return { label: `+${diff}${dayWord(diff)}`,          color: 'text-amber-500 bg-amber-50', overdue: false }
  const d = new Date(dueDate)
  return { label: `${d.getDate()}/${d.getMonth() + 1}`, color: 'text-slate-400 bg-slate-50', overdue: false }
}

// ─── Task Card ────────────────────────────────────────────────────────────────
// Mobile: true compact 2-row layout, ~62–70px per card, tap opens full TaskModal.
// Desktop: full card with inline edit, focus mode, goal strip, stuck panel, action buttons.
export default function TaskCard({ task, onMove, onDelete, onEdit, onUpdate, onToggleBig3, canAddBig3, isFocused, isDimmed, onFocus, goalOptions, isDragActive, dragListeners, dragHandleRef, onMobileMove }) {
  ...
  const [goalPopover,    setGoalPopover]    = useState(false)
  ...
  // Live elapsed timer for doing tasks
  useEffect(() => {
  ...
  function cancelInlineEdit() { setEditingTitle(false) }
  ...
  function handleComplete(e) {
    ...
    setCompleting(true)
    setTimeout(() => { setCompleting(false); onMove(task.id, 'done') }, 380)
  }
  ...
  const cardAnim  = isDragActive ? ''
                  : isBig3Active ? 'md:animate-glow-pulse'
                  : isDoing      ? 'md:animate-doing-ring'
                  : ''

  return (
    <div
      onClick={e => {
        if (!isDone && !e.target.closest('button') && !e.target.closest('input')) {
          // Mobile: tap opens full TaskModal (compact card has no inline detail expansion)
          if (window.innerWidth < 768) { if (onEdit) onEdit(task) }
          // Desktop: enter focus mode on board
          else if (onFocus) onFocus(task.id)
        }
      }}
      className={[
    ...
    >
      {/* Priority strip — thicker on mobile for visual punch */}
      <div className={`h-[3px] md:h-1 w-full ${priorityStripColor}`} />

      {/* ─────────────────────────────────────────────────────────────────────
          MOBILE COMPACT LAYOUT — hidden on md:+ screens
          Target: 62–70px total card height.
          Tap card → opens TaskModal. All details accessible via modal.
          No action strip row — primary action lives inline in title row.
      ───────────────────────────────────────────────────────────────────── */}
      <div className="md:hidden px-2.5 pt-2 pb-2">

        {/* Row 1: complete ● + title (truncated) + primary action button */}
        <div className="flex items-center gap-1.5">

          {/* Quick-complete circle — visual 22px, touch area padded */}
          <button
            onClick={handleComplete}
            className="flex items-center justify-center w-7 h-7 shrink-0 -mx-0.5"
            aria-label={isDone ? 'החזר למשימות' : 'סמן כבוצע'}
          >
            <div className={[
              'w-[22px] h-[22px] rounded-full border-2 flex items-center justify-center transition-all duration-200',
              isDone      ? 'bg-green-500 border-green-500'
              : completing ? 'bg-green-400 border-green-400'
              :              'border-slate-300',
            ].join(' ')}>
              {(isDone || completing) && <Check className="w-3 h-3 text-white" />}
            </div>
          </button>

          {/* Title — truncates cleanly, Big3 star prefix */}
          <p className={[
            'flex-1 min-w-0 text-[13px] font-semibold leading-tight truncate',
            isDone ? 'line-through text-slate-400' : 'text-slate-800',
          ].join(' ')}>
            {task.isBigThree && !isDone && <span className="text-brand-400 text-[11px] mr-0.5">⭐</span>}
            {isStuck && !isDone && <span className="text-red-500 text-[11px] mr-0.5">{stuckSignal?.icon}</span>}
            {task.title}
          </p>

          {/* ─ Primary action — ONE button, context-aware ─ */}
          {!isDone && isBacklog && (
            <button
              onClick={e => { e.stopPropagation(); onMove(task.id, 'today') }}
              className="shrink-0 text-[10px] font-black px-2 py-1 rounded-lg bg-amber-500 text-white whitespace-nowrap active:scale-95 transition-transform"
            >→ היום</button>
          )}
          {!isDone && isWeek && (
            <button
              onClick={e => { e.stopPropagation(); onMove(task.id, 'doing') }}
              className="shrink-0 text-[10px] font-black px-2 py-1 rounded-lg bg-purple-600 text-white whitespace-nowrap active:scale-95 transition-transform"
            >⚡</button>
          )}
          {!isDone && isToday && (
            <button
              onClick={e => { e.stopPropagation(); onMove(task.id, 'done') }}
              className="shrink-0 text-[10px] font-black px-2.5 py-1 rounded-lg bg-green-500 text-white active:scale-95 transition-transform"
            >✓</button>
          )}
          {!isDone && isDoing && (
            <button
              onClick={e => { e.stopPropagation(); onMove(task.id, 'done') }}
              className="shrink-0 text-[10px] font-black px-2 py-1 rounded-lg bg-green-500 text-white whitespace-nowrap active:scale-95 transition-transform"
            >✓ סגור</button>
          )}
          {isDone && (
            <button
              onClick={e => { e.stopPropagation(); onMove(task.id, 'backlog') }}
              className="shrink-0 text-[10px] font-black px-2 py-1 rounded-lg bg-slate-100 text-slate-500 active:scale-95 transition-transform"
            >↩</button>
          )}
        </div>

        {/* Row 2: ultra-compact meta — emoji + priority + overdue + elapsed */}
        {/* Indented by complete button width (28px) so it reads under the title */}
        <div className="flex items-center gap-1 mt-1 ml-[28px]">
          <span className="text-xs leading-none">{tMeta.emoji}</span>
          <span className={`text-[9px] font-bold px-1 py-0.5 rounded-full leading-none ${pMeta.bg}`}>
            {pMeta.label}
          </span>
          {!isDone && task.dueDate && (() => {
            const ds = dueDateStatus(task.dueDate)
            return ds ? (
              <span className={`text-[9px] font-bold px-1 py-0.5 rounded-full leading-none ${ds.color}`}>
                {ds.label}
              </span>
            ) : null
          })()}
          {isDoing && task.doingStartedAt && (
            <span className="text-[9px] font-bold text-purple-600 bg-purple-50 px-1 py-0.5 rounded-full leading-none">
              ⏱ {fmtElapsed(elapsed)}
            </span>
          )}
          {isStuck && !isDone && (
            <span className="text-[9px] font-bold text-red-700 bg-red-50 px-1 py-0.5 rounded-full leading-none">
              תקוע
            </span>
          )}
        </div>

      </div>{/* end mobile compact */}


      {/* ─────────────────────────────────────────────────────────────────────
          DESKTOP FULL LAYOUT — hidden on mobile
          Unchanged from original: inline edit, focus mode, goal strip,
          stuck panel, doing timer, multi-action buttons.
      ───────────────────────────────────────────────────────────────────── */}
      <div className="hidden md:block">
        <div className="px-3 pt-2.5 pb-3 space-y-2.5">
          ...
          <div className="flex items-start gap-2">
            <button
              ...
              className={[
                ...
                isDone      ? 'bg-green-500 border-green-500 scale-100'
                : completing ? 'bg-green-400 border-green-400 scale-110'
            ...
            <button
              ...
              className={`shrink-0 transition-all duration-200 ${
                task.isBigThree ? 'text-brand-500 scale-110' : 'text-slate-300 hover:text-brand-400 hover:scale-110'
              }`}
          ...
          {/* Details block — visible when focused or stuck */}
          <div className={`space-y-1.5 ${isFocused || isStuck ? '' : 'hidden'}`}>
          ...
          {/* Doing: compact action row */}
          {isDoing && !isFocused && (
            <div className="flex items-center gap-2 pt-0.5">
              <button onClick={e => { e.stopPropagation(); onMove(task.id, 'done') }}
                className="flex-1 py-1.5 rounded-lg text-xs font-bold bg-green-500 hover:bg-green-600 text-white transition-colors flex items-center justify-center gap-1">
                <Check className="w-3 h-3" /> סגור
              </button>
              <button onClick={e => { e.stopPropagation(); onMove(task.id, 'today') }}
                className="py-1.5 px-3 rounded-lg text-xs font-bold bg-amber-50 hover:bg-amber-100 text-amber-700 transition-colors">
                ⏸ פאוז
              </button>
            </div>
          )}

          {/* Desktop focused / done: full action buttons */}
          <div className={`items-center gap-2 pt-1 flex-wrap ${isFocused || isDone ? 'flex' : 'hidden'}`}>
            {isBacklog && (
              <button onClick={e => { e.stopPropagation(); onMove(task.id, 'today') }}
